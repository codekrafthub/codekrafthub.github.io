...
const WHATSAPP_SVG = (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="white" aria-hidden focusable="false">
...
export default function WhatsAppButton() {
  ...
  function handleClick() {
    ...
    if (isMobile) {
      if (expanded) {
        window.open(`whatsapp://send?phone=${PHONE}&text=${MESSAGE}`, '_blank', 'noopener,noreferrer');
  ...
  return (
    <>
      {/*
        <button> — correct semantic element for a JS-triggered action.
        No href="#" means no spurious page-top jump if JS fails to hydrate.
        Screen readers announce this as a button, not a link, which is accurate
        since it triggers an action (opening WhatsApp) rather than navigating.
      */}
      <button
        type="button"
        id="whatsappChatBtn"
      ...
      >
        <span className={styles.icon}>{WHATSAPP_SVG}</span>
        <span className={styles.label} aria-hidden>Chat with us</span>
      </button>

      {/*
        Fallback anchor rendered only when JS is disabled.
        Points to the real WhatsApp web URL so the button still works
        on no-JS browsers and in static HTML snapshots.
      */}
      <noscript>
        <a
          href={URL_WEB}
